const chainConfigDefinitions = {
  ...
  'project-unit-test-generation': {
    ...
    fields: [
      ...
      field('test.require-coverage', '要求覆盖率', '开启后才执行 JaCoCo 覆盖率验收；默认关闭。', 'checkbox'),
      field('test.coverage-threshold-percent', '覆盖率阈值', '要求覆盖率时，当前目标源码类达到该覆盖率百分比后当前 batch 才算达标。', 'number'),
      field('test.jacoco-version', 'JaCoCo 版本', '要求覆盖率时，run_command 验收测试使用的 JaCoCo agent/report 插件版本。'),
      field('test.jacoco-jvm-arg-property', 'JaCoCo JVM 参数属性', '要求覆盖率时，目标 Maven Surefire 实际读取的 JVM 参数属性；普通项目通常是 argLine。'),
      field('test.jacoco-jvm-arg-base', 'JaCoCo JVM 基础参数', '要求覆盖率时，可选；会和 -javaagent 一起写入上面的 Maven 属性。'),
