const chainConfigDefinitions = {
  ...
  'project-unit-test-generation': {
    ...
    fields: [
      ...
      field('test.jacoco-version', 'JaCoCo 版本', 'run_command 验收测试使用的 JaCoCo agent/report 插件版本。'),
      field('test.jacoco-jvm-arg-property', 'JaCoCo JVM 参数属性', '目标 Maven Surefire 实际读取的 JVM 参数属性；普通项目通常是 argLine。'),
      field('test.jacoco-jvm-arg-base', 'JaCoCo JVM 基础参数', '可选；会和 -javaagent 一起写入上面的 Maven 属性。'),
