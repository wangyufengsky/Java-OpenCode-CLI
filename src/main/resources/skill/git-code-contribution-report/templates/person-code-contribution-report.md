...
结合提交主题、Top 变更文件、扩展名分布和去注释代码行，分析该人员在统计期内更偏向：
...
| 文件 | 文件修改次数 | 原始新增/删除行 | 去注释新增/删除行 | 去注释代码变更量 | 分析 |
| --- | ---: | ---: | ---: | ---: | --- |
| `<path>` | `<file_change_count>` | `<added>` / `<deleted>` | `<non_comment_added>` / `<non_comment_deleted>` | `<non_comment_churn>` | `<analysis>` |
