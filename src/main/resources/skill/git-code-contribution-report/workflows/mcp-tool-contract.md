...
本 workflow 规定 `git-code-contribution-report` 在 Windows/OpenCode 运行环境中的 AgentBridge MCP 工具命名和文件读写约束。

OpenCode 配置中的 MCP server 名必须包含：
...
- AgentBridge 文件读取：`read_file`。
- AgentBridge 文件写入：`edit_text`、`write_file`。
- AgentBridge 搜索/定位：`search_text`、`search_symbols`、`list_project_files`、`list_directory_tree`。
...
除统计准备脚本外，读取文件和写报告必须走 OpenCode 的 AgentBridge MCP 工具。对应 MCP 未暴露、调用失败或目标路径不受支持时，停止该步骤并向用户报告失败；不要改用 shell、PowerShell、Python 临时脚本或重定向读写报告。
...
| 行为 | 强制 MCP | 规则 |
| --- | --- | --- |
| 读取准备脚本输出、模板和 prompt | `read_file` | 主 agent 只读 `summary.json`、`index_inputs.json`、`index_inputs.tasks[].report_md` 指定的个人报告和 `index_inputs.tasks[].quality_summary_json` 指定的质量摘要；子 agent 只读输入的 `detail_json` 和个人报告模板，质量分析只用 `detail.changed_regions` 中的 hunk。禁止用 `cat`、`type`、`Get-Content` 或 Python 读取这些文件作为报告依据。 |
...
| 定位公共代码调用证据 | `search_symbols`、`search_text`、`list_project_files`；候选调用点文件内容用 `read_file` 读取 | 仅当判断公共代码、工具类代码复用价值时使用；优先用 AgentBridge 搜索/符号工具确认调用点，最多读取 5 个候选调用点文件。MCP 不可用或证据不足时写入 `unverified`，禁止改用 shell、grep、rg 或 Python 扫描全项目。 |
| 写个人报告和总报告 | `edit_text`、`write_file` | 只替换脚本预创建文件中的 marker。 |
| 搜索已生成文件 | `list_project_files`、`list_directory_tree`、`search_text` | 优先按 `index_inputs.json` 中的确定路径直接读取；只有路径异常或需要定位预创建文件时才搜索。 |
...
- 写中间块时，用 `edit_text` 将 exact marker 替换为“本次内容 + 原 marker”，保留同一个 marker 供下一块继续追加。
