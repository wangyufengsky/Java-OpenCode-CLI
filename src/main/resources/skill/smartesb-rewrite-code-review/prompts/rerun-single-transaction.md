...
8. 补读代码优先使用 `当前可用搜索能力`、`当前可用项目文件列表能力`、`当前可用搜索能力`、当前可用读取能力。
9. 搜索不到刚生成或刚修改的文件时，先尝试 `当前可用项目文件列表能力`，再重试一次。
10. 需要数据库/SQL 证据时，优先使用当前客户端暴露的 `intellij-db_*` 工具；未暴露时记录未验证，不要用 shell 强行连库。
11. 写文件使用当前 AgentBridge 环境可用能力：当前可用写入能力、当前可用写入能力。任一目标文件缺失时返回 `无法完成` 和缺失路径，让主 agent 重新运行准备脚本；不得用 当前可用写入能力 创建缺失输出。AgentBridge MCP 不可用或路径不可写时，停止补审并报告失败；不要扩大任务边界、本地脚本或临时重定向写报告。
