...
13. 写文件使用当前 AgentBridge 环境可用能力 文件编辑工具：当前可用写入能力、当前可用写入能力。任一目标文件缺失时写入失败说明 和缺失路径，让 Java 调度器重新运行准备脚本；不得用 当前可用写入能力 创建缺失输出。`AgentBridge` MCP 读写工具不可用或路径不可写时，停止补审并报告失败；禁止使用 shell、本地脚本或临时重定向写报告。
