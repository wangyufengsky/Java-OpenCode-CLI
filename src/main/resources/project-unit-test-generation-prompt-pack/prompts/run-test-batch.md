...
- 只允许修改 `allowed_write_globs` 中当前模块的测试文件；仅在修复当前批次测试的编译或运行问题确有必要时，才允许修改当前模块 `pom.xml`，且该 POM 必须明确列在 `allowed_write_globs` 中。
...
- 只需修改当前批次允许范围内的测试文件或当前模块 `pom.xml`；Java 会在本轮结束后验收测试存在性、编译和运行。只有 `batch_input_json.coverage.required` 为 `true` 时才验收覆盖率。
- 目标项目使用 Maven，禁止运行 Gradle 或 `gradlew`；如确需执行构建或测试命令，只能使用 Maven。
- 最终只回复简短完成信息。
