export type ConsoleFieldType = 'text' | 'number' | 'checkbox' | 'textarea' | 'list' | 'path'
...
export interface ConsoleChainDefinition {
  label: string
  description: string
  fields: ConsoleFieldDefinition[]
}

export interface ConsoleRerunType {
  value: string
  label: string
  requiresId: boolean
...
export interface ConsoleGroupDefinition {
  group: string
  label: string
  description: string
}
...
export const groupDefinitions: ConsoleGroupDefinition[] = [
...
export const chainConfigDefinitions: Record<string, ConsoleChainDefinition> = {
  ...
  'mybatis-sql-review': {
    ...
    fields: [
      ...
      field('source.paths', 'Mapper 审查目录', '必填；每行一个相对 project.repo 的目录，只递归审查这些目录下的 Mapper XML。', 'list', 'scope', true, true),
      field('source.include', 'Mapper 包含路径', '每行一个相对审查目录的 XML include glob；至少保留一个 mapper 范围。', 'list', 'scope', true, true),
      field('source.exclude', 'Mapper 排除路径', '每行一个相对审查目录的 exclude glob，用于过滤生成文件。', 'list', 'scope'),
...
export const rerunTypeDefinitions: Record<string, ConsoleRerunType[]> = {
  'git-code-contribution-report': [
